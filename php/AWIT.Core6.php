<?php // _2019_04_17_21_21_21_43
	...
	define('self', dirname(__FILE__).DS);
	define('ACdir', dirname(self).DS);
	define('corePath_sys', ACdir.'styles'.DS);
	
	$SSF = $_SERVER['SCRIPT_FILENAME'];
	$SSF = str_replace(basename($_SERVER['SCRIPT_FILENAME']), '', $SSF);
//	echo $SSF;
	
	$ACPath = dirname(dirname(__FILE__));
	$ACPath = str_replace('\\', '/', $ACPath);
//	echo $ACPath;
	
	$ACPath = str_replace($SSF, '', $ACPath);
	$ACPath = $ACPath.'/';
//	echo $ACPath;
	
	define('ACPath', $ACPath);
	
	// CJBuild folder
	define('corePath_sys_cjbuild', corePath_sys.'CJBuild'.DS);
	define('corePath_sys_cjbuild_js', corePath_sys_cjbuild.'js'.DS);
	define('corePath_sys_cjbuild_css', corePath_sys_cjbuild.'css'.DS);
	
	// prebuild js, css
	define('coreFile_prebuild_js', corePath_sys_cjbuild.'prebuild_js.json');
	define('coreFile_prebuild_css', corePath_sys_cjbuild.'prebuild_css.json');
	
	// date js, css
	define('coreFile_date_js', corePath_sys_cjbuild_js.'!date_js.txt');
	define('coreFile_date_css', corePath_sys_cjbuild_css.'!date_css.txt');
	
	// build file name
	define('coreFile_build_name', 'build');
	
	// build js, css
	define('coreFile_build_js', corePath_sys_cjbuild_js.coreFile_build_name);
	define('coreFile_build_css', corePath_sys_cjbuild_css.coreFile_build_name);
	...
	include self.'AWIT.Core6.config.php';
	
	if ($BaseStyle) {
		array_unshift($AC_addCJ , ACPath.'styles/base/base.min.css');
	}
	if ($FontAwesome) {
		$AC_addCJ[] = ACPath.'styles/base/fonts/fontawesome-free-5.4.1-web/css/all.min.UP.css';
	}
	...
	if (!file_exists(corePath_sys)) {
		mkdir(corePath_sys, 0777, true);
	}
	if (!file_exists(corePath_sys_cjbuild)) {
		mkdir(corePath_sys_cjbuild, 0777, true);
	}
	if (!file_exists(corePath_sys_cjbuild_js)) {
		mkdir(corePath_sys_cjbuild_js, 0777, true);
	}
	if (!file_exists(corePath_sys_cjbuild_css)) {
		mkdir(corePath_sys_cjbuild_css, 0777, true);
	}
	...
	// CLASS & FUNCTIONS MINIFY
	class JSqueeze { /* Copyright (C) 2016 Nicolas Grekas - p@tchwork.com
	...
	// FUNCTIONS CLEAN BUILD FOLDERS
	function cleanBuildJsFolder() { // _2019_02_05_23_00_50_60
		$dir = corePath_sys_cjbuild_js;
		foreach(glob($dir.'*.*') as $v){
			unlink($v);
		}
	}
	function cleanBuildCssFolder() { // _2019_02_05_23_00_50_60
		$dir = corePath_sys_cjbuild_css;
		foreach(glob($dir.'*.*') as $v){
			unlink($v);
		}
	}
	...
	// FUNCTIONS DATE
	function newDate() { // _2019_02_05_07_16_39_08
		
//		echo dateFile;
		date_default_timezone_set('Europe/Kiev');
		$t = explode(" ",microtime());
		$t = date("_Y_m_d_H_i_s_",$t[1]).substr((string)$t[0],1,4);
		$t = str_replace(".","",$t);
		
		return $t;
		
	}
	function createDateFile($date) { // CREATE BUILD DATA FILE _2019_02_05_10_47_21_28
		
		$newDateFile = fopen(dateFile, "w") or die("Unable to open file!");
			fwrite($newDateFile, $date);
		fclose($newDateFile);
		
	}
	function createJsDateFile($date) { // _2019_02_05_22_57_29_97
		
		$newJsDateFile = fopen(coreFile_date_js, "w") or die("Unable to open file!");
			fwrite($newJsDateFile, $date);
		fclose($newJsDateFile);
		
	}
	function createCssDateFile($date) { // _2019_02_05_22_57_29_97
		
		$newCssDateFile = fopen(coreFile_date_css, "w") or die("Unable to open file!");
			fwrite($newCssDateFile, $date);
		fclose($newCssDateFile);
		
	}
	function getJsDateFile() { // _2019_02_05_22_57_29_97
		
		return file_get_contents(coreFile_date_js);
		
	}
	function getCssDateFile() { // _2019_02_05_22_57_29_97
		
		return file_get_contents(coreFile_date_css);
	...
	$filesCss = array();
	$filesJs = array();
	foreach ($AC_addCJ as $file) {
	
		preg_match_all('#(.*).css#is', $file, $matchesCss);
		foreach ($matchesCss[0] as $value) {
			$filesCss[] = $value;
		}
		
		preg_match_all('#(.*).js#is', $file, $matchesJs);
	...
	function BUILD($type, $data) {
		...
		if ($type == 'js') {
			...
			$coreFile_prebuild_type = coreFile_prebuild_css;
			$coreFile_build_type = coreFile_build_css;
			$ext = cssExt;
		}
		else {
			return;
		...
		$jsonUpdateStatus[] = 0;
		$json = array();
		if (file_exists($coreFile_prebuild_type)) {
			
			$json = file_get_contents($coreFile_prebuild_type);
	...
	//		print_r($json);
		...
		// IF NAME MATCH - UPDATE CONTENT & UPDATE JSON
		foreach($data as $file){
			
			foreach ($json as $key => $entry) {
				
				if ($entry->name == $file) { // search $file name in json file
					
					$fileContent = file_get_contents($file);
					
					if ($json[$key]->content !== $fileContent) { // if $file content == content in json file
					
						$json[$key]->content = $fileContent; // update content in json file where $file name
						$jsonUpdateStatus[] = true;
		...
		// get names from file json
		$jsonNames = array();
		foreach ($json as $key => $entry) {
			$jsonNames[] = $entry->name;
		}
		...
		// compare filesJs & jsonNames
		// find new files
		// if no match filesJs vs jsonNames
		$jsonNoMatchNames = array_diff($data, $jsonNames);
		
		foreach ($jsonNoMatchNames as $val) {
			$json[] = ['name' => $val, 'content' => file_get_contents($val)];
			$jsonUpdateStatus[] = true;
		}
		...
		// UPDATE JSON
		if (in_array("1", $jsonUpdateStatus)) { // one and only one true in the array
			
			// create date of last update
			$json[] = ['name' => 'lastUpdate', 'content' => newDate()];
			
			// create json file
			$json = json_encode($json);
			file_put_contents($coreFile_prebuild_type, $json);
			$json = json_decode($json);
		...
		foreach ($json as $key => $entry) {
			...
			if ($entry->name == "lastUpdate") {
				$lastUpdateDate = $entry->content;
			}
		...
		// BUILD JS, CSS FILES
		if (in_array("1", $jsonUpdateStatus)) {
	...
	//		print_r($jsonContent);
			...
			// create file
			file_put_contents($coreFile_build_type.$lastUpdateDate.$ext, $jsonContent);
			
		}
		
		
		
		return coreFile_build_name.$lastUpdateDate.$ext;
		
		
		
	}
  	$jsFileDone = BUILD('js', $filesJs);
	$cssFileDone = BUILD('css', $filesCss);
	...
	$done = "
		...
		"."$"."AWITCore({
			...
			BaseStyle: ".$BaseStyle.",
			CustomStylesScripts: [
				'".$doneFilesPath.'css/'.$cssFileDone."',
				'".$doneFilesPath.'js/'.$jsFileDone."',
			],
